...
def collate_fn(batch):
    ...
    batch[0] = nested_tensor_from_videos_list(batch[0], size_divisibility=32) # padding
    if "global_imgs" in batch[1][0]:
        batch[1][0]["global_imgs"] = batch[1][0]["global_imgs"].unsqueeze(0)
        batch[1][0]["global_imgs"] = nested_tensor_from_videos_list(batch[1][0]["global_imgs"], size_divisibility=32)
    if "local_imgs" in batch[1][0]:
        batch[1][0]["local_imgs"] = batch[1][0]["local_imgs"].unsqueeze(0)
        batch[1][0]["local_imgs"] = nested_tensor_from_videos_list(batch[1][0]["local_imgs"], size_divisibility=32)
        batch[1][0]["local_masks"] = batch[1][0]["local_masks"].unsqueeze(0).unsqueeze(2)
        batch[1][0]["local_masks"] = nested_tensor_from_videos_list(batch[1][0]["local_masks"].float(), size_divisibility=32)
...
def init_distributed_mode(args):
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        # len(os.environ['HIVED_VISIBLE_DEVICES'].split(",")) => torch.cuda.device_count()
        # os.environ['RANK'] = str(int(os.environ['PAI_TASK_INDEX']) * len(os.environ['HIVED_VISIBLE_DEVICES'].split(","))  + args.local_rank)
...
def targets_to(targets: List[Dict[str, Any]], device):
    ...
    """
    return [{k: v.to(device)  for k, v in t.items() if k not in ["caption", "dataset_name", "original_id", "image_id", "Arg0", "verb", "Arg1", "Arg2", "ARGM_LOC", "ARGM_TMP", "ARGM_DIR", "ARGM_MNR", "ARGM_PRP", "chosen_frame_path"]} for t in targets]
